...
public class ManProcessServiceImpl implements ManProcessService {
	 ...
	 * //스케쥴러
	 * 
	 * @Scheduled(fixedDelay=10000) //10초마다 실행된다(10000). 5초마다 실행된다.(5000) public
	 * void selectProcessTimer() { // 주기적으로 실행될 processTimer
	 * 
	 * //주기적으로 진행공정 테이블을 조회해온다. 조회한 행들을 담아줄 배열 pList List<ManProcessVO> pList = new
	 * ArrayList<>(); pList = mapper.selectProcTable(); //
	 ...
	 * if((mapper.selectNextProc(processVO)).getManStarttime().equals("0") //다음 공정의
	 * 시작 시간이 0이면 && (mapper.selectPreManQnt(processVO)).getManQnt() != null //이전
	 * 공정의 작업완료량이 null이 아니면(값이 들어오면) ) { //다음공정 시작시간 update
	 * System.out.println("111"); System.out.println("현재 공정 찾아서 시간 값 추가");
	 ...
	 * 
	 * } // else
	 * if((mapper.selectNextProc(processVO)).getManStarttime().equals("0")) { ////
	 * System.out.println("***************eeeeee*******"); //
	 * mapper.updateNowProc(mapper.selectNextProc(processVO)); // }
	 * 
	 * //작업 목표량이 실제 작업 완료량보다 크면 10초당 생산량을 up.
	 * System.out.println("!*111*작업 목표량이 실제 작업 완료량보다 크면 10초당 생산량을 up"); if
	 * (Integer.parseInt(processVO.getManGoalqnt()) >
	 * Integer.parseInt(processVO.getManQnt())) { ManProcessVO secondQnt =
	 * mapper.selectSecondQnt(processVO); //10초당 생산량 조회
	 * 
	 * // System.out.println("222"); // System.out.println(secondQnt);
	 * 
	 * //기존의 생산량 + 10초당 생산량 => VO의 작업완료량에 set해주기
	 * processVO.setManQnt(String.valueOf((Integer.parseInt(secondQnt.
	 * getQntPer10Second()) + Integer.parseInt(processVO.getManQnt()) ) ) );
	 * 
	 * // System.out.println("스케쥴러 테스트2: "+processVO);
	 * 
	 * System.out.println("!*111*proc001이면 10초가 지났을 때 작업완료량 update");
	 * if(processVO.getProcCode().equals("PROC001")) { //proc001이면 10초가 지났을 때 작업완료량
	 * update // System.out.println("333"); mapper.updateSecondQnt(processVO); //
	 * System.out.println("진행공정 TEST11: "+ processVO); }
	 * 
	 * //proc001이 아니고, 현재 공정 앞전 공정의 생산수량이 있으면, //앞전 공정의 작업완료량이 무조건 뒷 공정보다 많으면,
	 * //10초가 지났을 때 작업완료량 update //
	 * System.out.println("!*111*10초가 지났을 때 작업완료량 update"); else
	 * if(mapper.selectPreManQnt(processVO) != null &&
	 * Integer.parseInt((mapper.selectPreManQnt(processVO)).getManQnt()) >=
	 * Integer.parseInt(processVO.getManQnt()) + (
	 * Integer.parseInt(secondQnt.getQntPer10Second()) ) ) { //
	 * System.out.println("진행공정 TEST22: "+ processVO); //
	 * if((mapper.selectNextProc(processVO).getManStarttime())==null) { //현재공정(vo)을
	 * 조회해왔을 때 거기의 startTime== null => sysdate 로 update //
	 * System.out.println("다음공정 찾기"); // mapper.updateNowProc(processVO); //
	 * System.out.println("444"); // } System.out.println("444");
	 * System.out.println(processVO.getProcCode() + " : " + processVO.getManQnt());
	 * mapper.updateSecondQnt(processVO);
	 * 
	 * } //작업 지시량 < 현재 공정의 축적된 10초당 uph (최종 작업량) //작업 지시량만큼 생산하고 끝내라. // else
	 * if(Integer.parseInt(mapper.selectManGoalQnt(processVO)) <= //
	 * Integer.parseInt(processVO.getManQnt()) + (
	 * Integer.parseInt(secondQnt.getQntPer10Second()) )) { // // }
	 * 
	 * }
	 ...
	 * Integer.parseInt(processVO.getManQnt())) {
	 * System.out.println("표시: "+processVO); mapper.updateEndTime(processVO);
	 * System.out.println("작업지시량만큼 update"); mapper.updateManQnt(processVO); }
	 * 
	 * }
	 * 
	 * }
